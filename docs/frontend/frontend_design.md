...
主布局采用可折叠材质侧边栏、悬浮顶栏和内容区；移动端切换为抽屉式导航。

```text
frontend/src/
├── api/             # Axios 实例及业务 API
├── assets/styles/   # 全局变量、主题和基础样式
├── components/      # 认证壳层、资源卡片、目录、命令等复用组件
...
平台默认采用 Apple 式界面语言，并保留暖米色、珊瑚橙的 SprIoT 品牌识别；支持亮暗模式和经典蓝色配色切换。颜色、实体表面、玻璃材质、排版、圆角、阴影、间距和动态响应集中在 CSS 变量中。

设计基础遵循以下约定：

- 系统字体与按字号变化的字距/行高，数据使用等宽数字。
- 顶栏、侧栏、弹窗和浮层使用分级半透明材质，较大表面拥有更强的模糊和阴影。
- 可操作元素在按下时立即反馈；非手势动效使用接近临界阻尼的缓动，进入与退出遵循同一路径。
- `AuthShell` 统一登录/注册界面，`ResourceCard` 统一传感器/设备摘要卡。
- `prefers-reduced-motion`、`prefers-reduced-transparency` 和 `prefers-contrast` 均有全局回退。
- 旧的本地配色值 `claude` 会自动迁移为 `apple`，内部仍保留 `classic` 选项。
